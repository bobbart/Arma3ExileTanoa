...
UPDATEEMAIL='bobbartholomewjr@gmail.com';
INFISTARVERSION='13-11-2017 13-18-48 - v87';
...
diag_log format['<infiSTAR.de> %1 - VERSION: 13-11-2017 13-18-48 - v87',time];
{
	_x params ['_uids'];
	{
		private _uid = _x;
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [29274x2 - 13-11-2017 13-18-48 - v87 - %2 - %3]',time,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!   [29274x2 - 13-11-2017 13-18-48 - v87 - %2 - %3]',time,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = "
	...
	diag_log format['<infiSTAR.de>%1| %2   [13-11-2017 13-18-48 - v87]',_logname,_logentry];
";
...
_fnc_RandomGen =
{
	...
	_fnc_actualGen = {
		...
		_main	= ['a','8','f','a','2','b','6','6','d','7','b','d','4','4','b','4','7','2','4','5','c','e','8','3','2','2','5','8','0','3','e','b','7'];
...
_m = missionNameSpace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=14637';missionNameSpace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
_url = _url - [35];
_url = toString _url;
_url = 'http://load.infistar.de/rp.php?steamid=' + _url;
_code = {
...
_fnc_finddeeper = {
	params[['_name',''],['_puid',''],['_AHKickLog',{(findDisplay 46)closeDisplay 0;}],['_AHKickOFF',{(findDisplay 46)closeDisplay 0;}],['_var',''],['_tofindvar',[]],['_tofindcontent',[]],['_tofindboth',[]]];
	if(_var isEqualTo '')exitWith{};
	_lvar = toLower _var;
	_bv = missionNamespace getVariable _var;
	if(!isNil '_bv')then
	{
		if(_bv isEqualTo -4)exitWith{};
	...
	if(alive _origunit)then
	{
		if!(player isEqualTo _origunit)then
		{
			...
			if(!isNil'_ExileXM8IsOnline')then
			{
				...
				[_name,_puid,'SLOG_RE',toArray(_log)] call _AHKickLog;
				if(alive _origunit)then{deleteVehicle _origunit;};
			};
		};
	};
	_origunit = player;
...

{
	...
	if(!isNil'_actionParams')then
	{
		...
		"; if(_BadActionContentCheck)then{ _asd = _asd + "
			private _actionParamsString = toLower(str _actionParams);
			{
				_cCheckArray = _x;
				{
			...
			} forEach [['removegoggles player;','player setface','player setspeaker','call life_','spawn life_'], _tofindvar, _tofindcontent, _tofindboth, _verybadStrings];
...
else
{
	...
	if(!_traderDialogIsInitialized)then
	{
		...
		_log = format[
			'Possible Hackmenu found: idd: %1, parent idd: %2, controls: %3, controls parent: %4, control texts: %5',
		...
		while{true}do
		{
			...
			if(alive player)then
			{
				...
				if(isNull objectParent player)then
				{
					...
					{
						_log = format['BadRecoil %1 | %2 %4',_curecoil,_type,currentWeapon player];
						[_name,_puid,'HLOG_SKICK',toArray(_log)] call _AHKickLog;
						[] call _AHKickOFF;
					};
					"; }; _asd = _asd + "
...
_devLog = format['infiSTAR.de AHAT %1 - 13-11-2017 13-18-48 - v87 - server running: %2:29274x2',VERSIONCHECKRESULT,time call FN_GET_TIME_TIME];diag_log _devLog;
"; if(!_HIDE_FROM_PLAYERS)then{ _asd = _asd + "
...
if(isNil'i_cfgNames')then
{
	...
	i_cfgNames = compileFinal (str _cfgNames);
	i_onEventsArray = compileFinal (str _onEventsArray);
};
_cfgNames = call i_cfgNames;
_onEventsArray = call i_onEventsArray;

[
	[_name,_puid,_AHKickLog,_AHKickOFF,_onEvents,_cfgNames,_onEventsArray],
	{
...
fn_infiSTAR_checkGlobalBanState = {
	...
	"; if(_USE_GLOBAL_BAN_CHECK)then{ _asd = _asd + "
		...
		"; }; _asd = _asd + "
	"; }else{ _asd = _asd + "
		_log = format['%1(%2) | GLOBAL BANNED PLAYER (NOT BANNED LOCALLY) | %3 [13-11-2017 13-18-48 - v87]',_name,_uid,time call FN_GET_TIME_TIME];
	...
	if(_what isEqualTo 'NR')exitWith
	{
		[
			[_clientName,_clientUID],
			{
				...
				disableSerialization;
				_url = format['http://htmlload.infistar.de/rp.php?steamid=%1&name=%2&servername=%3',_clientUID,_clientName,serverName];
...
"; }; _asd = _asd + "
	...
	_log = format['%1(%2) | %3 | %4 [13-11-2017 13-18-48 - v87]',_name,_uid,_log,time call FN_GET_TIME_TIME];
...

{
	...
	"; }; _asd = _asd + "
		...
		"; if(_PlayerScoreList)then{ _asd = _asd + "
			_xo setVariable ['ExileKills', _xo getVariable ['ExileKills', 0], true];
			_xo setVariable ['ExileDeaths', _xo getVariable ['ExileDeaths', 0], true];
		"; }; _asd = _asd + "
		...
		_var = [_x] spawn {
			...
			while {true} do
			{
				...
				if(PlayerScoreList_shown)then
				{
					...
					_stats = [];
					{
						_kills = _x getVariable ['ExileKills', 0];
						_deaths = _x getVariable ['ExileDeaths', 0];
