...
VERSION_DATE_IS = '13-11-2017 13-18-48';
...
fnc_initOnKeyPress = {
	...
	_ctrl ctrlSetText format['Players loaded in: %1 of %2                 SERVER UPTIME: %3  SERVER FPS: %4  SERVER THREADS: %5          infiSTAR.de Admin Menu 13-11-2017 13-18-48 - v87',count (call fnc_get_plr),((playersNumber west)+(playersNumber east)+(playersNumber civilian)+(playersNumber resistance)),time call FN_GET_TIME_TIME,SERVER_FPS,SERVER_THREADS];
...
fnc_fill_infiSTAR_MAIN = {
	...
	if(FILLMAINSTATE == 0)then
	{
		...
		_ctrl lbAdd 'F2 - Territory Management (if you have the addon!)';
		_ctrl lbAdd 'F2 & SHIFT - configviewer';
		_ctrl lbAdd 'F3 - Adminconsole / Debug Console';
		_ctrl lbAdd 'F4 - Treelist Item Spawn Menu';
		_ctrl lbAdd 'F5 - Change Weather + View Distance';
		...
		_ctrl lbAdd 'F9 & SHIFT- To hide/show specate overlay (while spectating)';
		_ctrl lbAdd 'F10 - Stop Spectating';
		_ctrl lbAdd 'F11 - Add Ammo for current weapon';
		_ctrl lbAdd '1 & CTRL - Light / Zeus';
		_ctrl lbAdd '4 & SHIFT - Fly Up';
		_ctrl lbAdd '4 & CTRL - Teleport Up';
		_ctrl lbAdd '5 & SHIFT - Teleport in looking direction (if enabled)';
		_ctrl lbAdd '7 - Unlock/Lock targeted Vehicle';
		_ctrl lbAdd '7 - Opens/Closes targeted Door/Hatch/Gate';
		_ctrl lbAdd 'B & SHIFT - Hover on position';
		_ctrl lbAdd 'TAB & SHIFT - Open Map';
		_ctrl lbAdd 'I & SHIFT - Show Info (Like Codes of Vehicles and Doors)';
...
admin_showinfo_catch = {
	...
	delete_old_show_thread = _this spawn {
		...
		while {_timer > diag_tickTime} do
		{
			...
			_ctrl ctrlSetPosition [safeZoneX + safeZoneW - 1.2,safeZoneY + 0.075,0.6,(0.4)+(((count _buildRightsUids)+2)/40)];
			_ctrl ctrlCommit 0;
			_txt = format['
				...
				<t align=''left'' size=''.9'' color=''#44CD00'' shadow=''1'' shadowColor=''#000000''>model info: </t><t align=''left'' size=''.9'' color=''#5FBEDE'' shadow=''1'' shadowColor=''#000000''> %8</t><br/>
			',
				_type,
				_pinshown,
				_ownedby,
...
fnc_get_inputText = {
	...
	_ctrl ctrlSetEventHandler['ButtonClick','
		_text = ctrlText ((findDisplay -1341) displayCtrl 1336);
		_function = missionNameSpace getVariable['+str _function+',{}];
		[_text,'+str _input+'] call _function;
	true;'];
	_ctrl ctrlCommit 0;
...
infiSTAR_FlyUp = {
	_obj = cameraOn;
	if(!local _obj)exitWith{};
	if((_keyshift)||(_obj isKindOf 'Air'))exitWith
	{
		_vel = velocity _obj;
...
fnc_infiAdminKeyDown = {
	...
	_return = false;
	if(_keycode isEqualTo OPEN_ADMIN_MENU_KEY)then
	{
		FILLMAINSTATE=0;
		[] call fnc_FULLinit;
	};
	if(_keycode isEqualTo 0x3B && !(OPEN_ADMIN_MENU_KEY isEqualTo 0x3B))then
	{
	...
	if(_keycode isEqualTo 0x3D)then
	{
		if(_keyshift)then
		{
			if(ALLOW_ME_THIS_KEYBIND)then
	...
	if(_keycode isEqualTo 0x3E)then
	{
		if('Items spawn menu' call ADMINLEVELACCESS)then
		{
	...
	if(_keycode isEqualTo 0x3F)then
	{
		if('Change ViewDistance' call ADMINLEVELACCESS)then
		{
			if(_keyshift || _keyctrl || _keyalt)exitWith{};
			call FN_CHANGE_VIEWDISTANCE;
		};
	};
	if(_keycode isEqualTo 0x17)then
	{
		if(('showinfo' call ADMINLEVELACCESS)&&(_keyshift))then
		{
			if(!isNull cursorTarget)then
			{
				[] spawn admin_showinfo;
			};
		};
	};
	if(_keycode isEqualTo 0xD3)then
	{
		if('Delete Vehicle' call ADMINLEVELACCESS)then
		{
			[''] call fnc_deleteVeh_selected;
		};
	};
	if(_keycode isEqualTo 0x42)then
	{
		if('Flip Vehicle' call ADMINLEVELACCESS)then
		{
			[''] call fnc_flipVeh;
		};
	};
	if(_keycode isEqualTo 0x02)then
	{
		if(ALLOW_ME_THIS_KEYBIND || 'Light' call ADMINLEVELACCESS)then
		{
			if(_keyctrl)then{[''] call fnc_Light_selected; };
		};
	};
	if(_keycode isEqualTo 0x03)then
	{
		if(ALLOW_ME_THIS_KEYBIND)then
		{
			if(_keyctrl)then
	...
	if(_keycode isEqualTo 0x05)then
	{
	...
	if(_keycode isEqualTo 0x06)then
	{
		if(_keyshift)then
		{
			[] call infiSTAR_Tpdirection;
		};
	};
	if(_keycode isEqualTo 0x08)then
	{
		if('UnlockLockVehicle' call ADMINLEVELACCESS)then
		{
			[] call infiSTAR_A3Togglelock;
		};
	};
	if(_keycode isEqualTo 0x43)then
	{
		if(_keyshift)then
		{
	...
	if(_keycode isEqualTo 0x44)then{ call fnc_endspectate; };
	if(_keycode isEqualTo 0x2F)then
	{
		if(ALLOW_ME_THIS_KEYBIND)then
		{
			if(_keyshift)then
			...
			if(_keyctrl)then
			{
				[] call infiSTAR_go_down;
			};
		};
	};
	if(_keycode isEqualTo 0x30)then
	{
		if(_keyshift)then{ [] call fnc_Hover; };
	};
	if(_keycode isEqualTo 0x40)then
	{
		if('HealSelf' call ADMINLEVELACCESS)then
		{
			[] call infiSTAR_A3Heal;
			'HealSelf' call fnc_adminLog;
			playsound 'AddItemOK';
		};
	};
	if(_keycode isEqualTo 0x41)then
	{
		if('HealRepairNear' call ADMINLEVELACCESS)then
		{
			[] call infiSTAR_A3RestoreNear;
			'HealRepairNear' call fnc_adminLog;
			playsound 'AddItemOK';
		};
	};
	if(_keycode isEqualTo 0x0F)then
	{
		if(_keyshift)then
		{
	...
	if(_keycode isEqualTo 0x57)then
	{
	...
	if(_keycode isEqualTo 0x0E)then
	{
		if(!isNil'prevLoc')then
		{
			[1,netId player,prevLoc] call fn_ACR;
			prevLoc = nil;
		};
	};
	_return
...
_log = format['<infiSTAR.de> %1 - Menu Loaded - press F1 (default Key) to open it!',time call FN_GET_TIME_TIME];systemchat _log;diag_log _log;
