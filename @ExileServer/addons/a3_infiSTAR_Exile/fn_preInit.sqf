...
_response = "armalog" callExtension ("171adb3c59a3f&l=8fa2b66d7bd44b47245ce83225803eb7&sn="+str(toArray serverName));
...
if(_isWindows && !_armalog)exitWith{
	diag_log "<infiSTAR.de> Can not load infiSTAR";
	diag_log "<infiSTAR.de> 71adb3c59a3f";
	diag_log "<infiSTAR.de> 8fa2b66d7bd44b47245ce83225803eb7";
...
if(_isWindows && _armalog)then{
	...
	FN_ARMA_REMOTELOG = {
		...
		"armalog" callExtension ("9"+"exile"+"§§"+_logname+"§§"+_logentry+"§§"+servername+"§§13-11-2017 13-18-48 - v87§§bobbartholomewjr@gmail.com");
	};
...
_infiSTAR_RE_LIST append ["fn_infiSTAR_antiDupeServer","fn_infiSTAR_TM__SERVERHANDLE","fn_infiSTAR_privChatServer","fn_infiSTAR_antiVehicleTheft","fn_infiSTAR_serverOwner","fn_infiSTAR_lagSwitchServer","fn_friendList_HandleServer"];
...
/****************************************************************************************************/
_lagSwitchCheck = ["lagSwitchCheck",false] call fnc_infiSTAR_cfg;
/****************************************************************************************************/
...
fn_infiSTAR_lagSwitchServer = {
	...
	{
		_remoteExecutedOwner = remoteExecutedOwner;
		_serverPingCounter = missionNameSpace getVariable[format["serverPingCounter_%1",_remoteExecutedOwner],0];
		_serverPingCounter = _serverPingCounter + 1;
		missionNameSpace setVariable[format["serverPingCounter_%1",_remoteExecutedOwner],_serverPingCounter];
		
		_clientPingCounter = _this param [1,0,[0]];
		_clientServerTime = _this param [2,0,[0]];
		[_serverPingCounter,_clientPingCounter,serverTime,_clientServerTime] remoteExecCall ["fn_infiSTAR_lagSwitchClient",_remoteExecutedOwner,false];
	};
	if(_option isEqualTo 1)exitWith
	{
		...
		_serverPingCounter = _this param [4,0,[0]];
		_clientPingCounter = _this param [5,0,[0]];
		
		_pingCounterDif = _serverPingCounter - _clientPingCounter;
		if(_pingCounterDif > 5)then {
			[_name,_uid,remoteExecutedOwner,format["%1 pings could not reach the player..",_pingCounterDif]] spawn FNC_INFISERVERKICK;
		};
		
		_log = format["%1(%2) - Lagswitch? client did not respond to server for %3s. %4 pings successfully received since player connected AND %5 pings sent.",_name, _uid, _timeDiff, _clientPingCounter, _serverPingCounter];
		["LAGSWITCH_NEW",_log] call FNC_A3_CUSTOMLOG;
...
[
	"",
	{
		...
		uiSleep 2;
		if(!isNil"infiSTAR_LagSwitch_THREAD")then{terminate infiSTAR_LagSwitch_THREAD;};
		infiSTAR_LagSwitch_THREAD = [] spawn {
			_clientPingCounter = missionNameSpace getVariable[format["clientPingCounter_%1",clientOwner],0];
			while {true} do
			{
				_clientPingCounter = _clientPingCounter + 1;
				[4,[[0, _clientPingCounter, serverTime],"fn_infiSTAR_lagSwitchServer"]] call FN_infiSTAR_CS;
				sleep 5;
			};
		};
	}
] remoteExecCall ["spawn",-2,"LagSwitchServer_JIP"];

fn_infiSTAR_lagSwitchClient = {
	_serverPingCounter = _this param [0,0,[0]];
	_clientPingCounter = _this param [1,0,[0]];
	missionNameSpace setVariable[format["clientPingCounter_%1",clientOwner],_clientPingCounter];
	
	_serverServerTime = _this param [2,0,[0]];
	_clientServerTime = _this param [3,0,[0]];
	
	if(isNil "clientServerOffset")then
	{
		clientServerOffset = abs(_serverServerTime - _clientServerTime);
	}
	else
	{
		_timeDiff = abs(_serverServerTime - _clientServerTime);
		if((_timeDiff - clientServerOffset) > 3)then
		{
			["ErrorTitleAndText", ["NETWORK LAG DETECTED!",format["Ping > %1",_timeDiff]]] call ExileClient_gui_toaster_addTemplateToast;
			
			if((_timeDiff - clientServerOffset) > 5)then
			{
				if(!userInputDisabled)then{disableUserInput true;};
				
				[4,[[1, profileName, getPlayerUID player, _timeDiff, _serverPingCounter, _clientPingCounter],"fn_infiSTAR_lagSwitchServer"]] call FN_infiSTAR_CS;
			};
		}
		else
		{
...
publicVariable "fn_infiSTAR_lagSwitchClient";
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!   [1000MS - 13-11-2017 13-18-48 - v87 - %2 - %3]',time,serverName,productVersion];
...
[] spawn {
	[["https://dll.infistar.de/remote.php?key=8fa2b66d7bd44b47245ce83225803eb7"] call FN_ARMA_FETCHDATA] call {
		_this params [
			["_ret","",[""]]
		];
